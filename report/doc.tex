...
Χρησιμοποιούμε το ίδιο per-class subsampling με το Q2 (cap \(=25.000\)
στο default fast mode \(\to 134.187\) rows). Όλες οι rare classes
(Heartbleed, SQL Injection, Infiltration) διατηρούνται πλήρως.
...
\begin{itemize}
    ...
    προς τη διάσταση. Από \(47 \to 17\) components είναι \(\sim 3\times\)
    speedup.
\end{itemize}
...
\begin{figure}[H]
    ...
    cumulative (κόκκινη γραμμή). Με \textbf{17 components} καλύπτουμε
    \(\sim 87\%\) της variance· επιλέγουμε αυτό ως input για όλους τους
    αλγορίθμους clustering. Παρατηρήστε ότι ο πρώτος component
    συλλαμβάνει μόνο \(\sim 17\%\): δεν υπάρχει ένα κυρίαρχο axis. Η
...
\emph{Assignment} κάθε point στο πιο κοντινό centroid, (2)
\emph{Update} κάθε centroid ως μέσο όρο των points του. Συγκλίνει σε
local minimum του inertia (sum of squared distances). Υποθέτει
\textbf{σφαιρικά clusters παρόμοιου μεγέθους}.

\textbf{Επιλογή k - Elbow + Silhouette.} Σαρώνουμε \(k \in [2, 15]\).
Το inertia (αριστερός άξονας) μειώνεται σταθερά - δεν υπάρχει σαφής
\og αγκώνας\fg{}, αλλά το rate of decrease μειώνεται μετά το \(k=8\).
Το silhouette (δεξιός άξονας) έχει local minimum στο \(k=3\) και μετά
ανεβαίνει σταθερά μέχρι \textbf{maximum στο \(k=15\)}
(silhouette\,\(=0{,}4237\)).
...
\begin{figure}[H]
    ...
    \(k\). Το silhouette κορυφώνεται στο \(k=15\) - ο ίδιος αριθμός
    με τα γνωστά labels.}
    \label{fig:q3_kmeans_elbow}
\end{figure}

\textbf{Σύμπτωση.} Το silhouette-best \(k\) (\(=15\)) συμπίπτει με τον
αριθμό των γνωστών labels (\(=15\)). Δηλαδή το «reference k = \#true
labels» που ζητά η εκφώνηση δίνει \emph{ίδια} αποτελέσματα με το
silhouette-best k σε αυτή την περίπτωση. Αυτό από μόνο του είναι
ενθαρρυντικό σήμα ότι η ενδογενής δομή των δεδομένων αντιστοιχεί
στις γνωστές attack categories.
...
\begin{table}[H]
    ...
    \begin{tabular}{lrrr}
        ...
        \midrule
        K-Means \(k=15\) (best by silhouette / reference) & 15 & 0{,}4237 & 0{,}8278 \\
        \bottomrule
    \end{tabular}
    \caption{K-Means - test metrics στο best \(k\).}
    \label{tab:q3_kmeans}
\end{table}
...
\begin{figure}[H]
    ...
    \caption{K-Means \(k=15\) - PCA 2-D scatter. Αριστερά: colored by
    cluster. Δεξιά: colored by true Label. Παρατηρήστε ότι μερικές
    classes (DDoS, PortScan) είναι ευδιάκριτες ως compact clusters
    στο PCA space, ενώ η BENIGN καταλαμβάνει το πιο πολυποίκιλο
    γεωμετρικό χώρο.}
...
\begin{figure}[H]
    ...
    \caption{K-Means \(k=15\) - cluster composition heatmap.
    Row-normalized: κάθε γραμμή αθροίζει σε 1, δείχνοντας \emph{τι
    είναι} κάθε cluster. Πολλά clusters είναι σχεδόν καθαρά μίας
    κλάσης (\(>90\%\)), ενώ άλλα μοιράζονται μεταξύ similar attack
    types.}
...
ολόκληρο τον \(O(n^2)\) πίνακα αποστάσεων: για \(134\)K rows χρειάζονται
\(\sim 72\) GB RAM - απαγορευτικό. Παίρνουμε stratified subsample
\textbf{\(2.468\) rows} (\(\sim 70\) MB distance matrix) - αρκετό για
διαβάσιμο dendrogram και σταθερό linkage comparison.
...
\textbf{Linkage comparison} με \(k=15\) (= silhouette-best K-Means k,
για consistency):
...
\begin{table}[H]
    ...
    \begin{tabular}{lrr}
        ...
        \midrule
        ward       & 0{,}5014 & 0{,}5944 \\
        complete   & 0{,}6251 & 0{,}2924 \\
        \textbf{average}    & \textbf{0{,}7334} & \textbf{0{,}2185} \\
        \bottomrule
    \end{tabular}
    \caption{Hierarchical Clustering - σύγκριση linkage methods (\(k=15\),
    \(2.468\)-row stratified subsample). Best by both metrics: average
    linkage.}
    \label{tab:q3_hier}
\end{table}

\textbf{Παρατήρηση: γιατί κερδίζει το average linkage.} Συνήθως το
ward είναι το πιο «καλό default». Στο δικό μας dataset υπερισχύει το
average επειδή υπάρχουν λίγα μεγάλα και πολλά μικρά (rare-attack)
clusters με ασύμμετρες αποστάσεις - το average linkage, που
λαμβάνει υπόψη όλα τα ζεύγη points, χειρίζεται καλύτερα αυτή την
ασυμμετρία από το variance-based ward.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{q3_hier_best_scatter.png}
    \caption{Hierarchical (average linkage, \(k=15\)) - PCA 2-D scatter
...
\begin{figure}[H]
    ...
    \caption{Hierarchical (average linkage, \(k=15\)) - cluster
    composition heatmap.}
...
\begin{table}[H]
    ...
    \begin{tabular}{rrrrrr}
        ...
        \midrule
        0{,}3 & 5  & 583 & 6.494  & 0{,}3862 & 0{,}6359 \\
        0{,}3 & 10 & 367 & 9.052  & 0{,}4265 & 0{,}6406 \\
        0{,}3 & 20 & 231 & 12.573 & \textbf{0{,}4480} & 0{,}6354 \\
        0{,}5 & 5  & 402 & 4.018  & 0{,}3620 & 0{,}7432 \\
        0{,}5 & 10 & 258 & 5.692  & 0{,}3922 & 0{,}7290 \\
        0{,}5 & 20 & 172 & 7.836  & 0{,}4321 & 0{,}7479 \\
        0{,}8 & 5  & 295 & 2.473  & 0{,}0892 & 0{,}6930 \\
        0{,}8 & 10 & 191 & 3.545  & 0{,}1007 & 0{,}7386 \\
        0{,}8 & 20 & 122 & 5.183  & 0{,}2819 & 0{,}7062 \\
        1{,}0 & 5  & 184 & 1.868  & 0{,}2156 & 0{,}5536 \\
        1{,}0 & 10 & 123 & 2.624  & 0{,}2237 & 0{,}5643 \\
        1{,}0 & 20 & 88  & 3.696  & 0{,}2584 & 0{,}6136 \\
        1{,}5 & 5  & 142 & 1.006  & 0{,}1069 & 0{,}5095 \\
        1{,}5 & 10 & 89  & 1.512  & 0{,}1021 & 0{,}4745 \\
        1{,}5 & 20 & 55  & 2.178  & 0{,}1831 & \textbf{0{,}4513} \\
        \bottomrule
    \end{tabular}
    \caption{DBSCAN sweep - όλες οι 15 παραλλαγές. Best silhouette:
    \(\varepsilon=0{,}3, \texttt{min\_samples}=20\). Best Davies-Bouldin:
    \(\varepsilon=1{,}5, \texttt{min\_samples}=20\). Διαφορετικές
    μετρικές δίνουν διαφορετικό best - κλασικό clustering trade-off.}
    \label{tab:q3_dbscan}
\end{table}
...
\begin{itemize}
    ...
    (55-583), πολύ περισσότερα από τα 15 πραγματικά labels. Αυτό
    σημαίνει ότι η ενδογενής δομή έχει πολλές sub-populations -
    διαφορετικά \emph{flavors} του ίδιου attack type ή του BENIGN.
    \item \textbf{Trade-off noise vs cluster purity}: μικρό
    \(\varepsilon\) \(\Rightarrow\) πολύ noise (12K rows με
    \(\varepsilon=0{,}3\)) αλλά καλύτερο silhouette. Μεγάλο
    \(\varepsilon\) \(\Rightarrow\) λίγο noise αλλά \og θολά\fg{}
    clusters.
    \item Επιλέγουμε ως \og best\fg{} το \(\varepsilon=0{,}3,
    \texttt{min\_samples}=20\) (highest silhouette, μεσαίο noise level).
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{q3_dbscan_best_scatter.png}
    \caption{DBSCAN (\(\varepsilon=0{,}3,\,\texttt{min\_samples}=20\)) -
    PCA 2-D scatter. Τα grey x-markers είναι \emph{noise points}
    (\(12.573\) rows, \(\sim 9{,}4\%\) του dataset).}
...
\begin{table}[H]
    ...
    \begin{tabular}{llrrrr}
        ...
        \textbf{DB \(\downarrow\)} \\
        \midrule
        K-Means      & \(k=15\) (best/reference)         & 15 & 0      & 0{,}4237 & 0{,}8278 \\
        Hierarchical & ward, \(k=15\)                    & 15 & 0      & 0{,}5014 & 0{,}5944 \\
        Hierarchical & complete, \(k=15\)                & 15 & 0      & 0{,}6251 & 0{,}2924 \\
        Hierarchical & \textbf{average, \(k=15\)}        & 15 & 0      & \textbf{0{,}7334} & \textbf{0{,}2185} \\
        DBSCAN       & \(\varepsilon=0{,}3, ms=20\)      & 231 & 12.573 & 0{,}4480 & 0{,}6354 \\
        DBSCAN       & \(\varepsilon=1{,}5, ms=20\)      & 55 & 2.178  & 0{,}1831 & 0{,}4513 \\
        \bottomrule
    \end{tabular}
    \caption{Q3 - σύγκριση best configurations ανά αλγόριθμο. \emph{Caveat}:
    το Hierarchical εκπαιδεύτηκε σε \(2.468\)-row subsample λόγω
    \(O(n^2)\) memory.}
...
\textbf{Μερικώς, ναι.} Από τα cluster-to-Label heatmaps βλέπουμε ότι
αρκετά clusters είναι σχεδόν καθαρά μίας κλάσης - ιδίως οι
χαρακτηριστικές attack υπογραφές (DDoS, DoS Hulk, PortScan,
FTP/SSH-Patator). Όμως:
\begin{itemize}
    \item Η \texttt{BENIGN} \emph{διασκορπίζεται σε πολλά clusters} -
    δεν είναι ομοιογενής class αλλά μείγμα από web/email/SSH/bulk traffic
    με διαφορετικές υπογραφές. Είναι λογικό: «κανονικό» traffic έχει πολλές
    νόμιμες μορφές.
    \item Οι rare attacks (Heartbleed, SQL Injection, Infiltration)
    σπάνια έχουν δικό τους cluster· τυπικά απορροφώνται μέσα σε
    BENIGN clusters ή σε άλλες DoS-like ομάδες.
    \item Επιβεβαίωση: το silhouette-best \(k\) για το K-Means είναι
    ακριβώς 15, ίδιο με τα true labels. Αυτό από μόνο του είναι ισχυρό
    σήμα ότι η ενδογενής γεωμετρία αντιστοιχεί στις attack categories.
\end{itemize}

\paragraph{2. Ποιος αλγόριθμος συλλαμβάνει καλύτερα την true structure;}
\textbf{Hierarchical με average linkage} (silhouette\,\(=0{,}733\),
DB\,\(=0{,}219\)) έχει καθαρά τα καλύτερα νούμερα. Ωστόσο τρέχει σε
δραματικά μικρότερο sample (\(2{,}5\)K rows έναντι \(134\)K), και
άρα τα metrics δεν είναι 1:1 συγκρίσιμα. Σε ίσο sample:
\begin{itemize}
    \item \textbf{K-Means \(k=15\)}: silhouette \(\sim 0{,}42\) -
    σταθερό baseline.
    \item \textbf{DBSCAN best}: silhouette \(\sim 0{,}45\) - οριακά
    καλύτερο, αλλά εις βάρος του να αφήνει \(12.5\)K rows ως noise.
\end{itemize}
Με μετρικές που δίνουν ισότιμη σημασία και στο coverage (πόσα rows
καταλήγουν σε valid cluster) και στην ποιότητα των clusters,
\textbf{το K-Means είναι το πιο πρακτικό} σε αυτό το dataset: σαφές
ranking στο silhouette, καμία row ως noise, σταθερά μετρικά.
...
\begin{itemize}
    \item \textbf{Sub-clusters μέσα σε γνωστά labels.} Π.χ. το BENIGN
    χωρίζεται σε \(>5\) clusters - διαφορετικοί τύποι κανονικού
    traffic. Αυτό σημαίνει ότι στο supervised setting, ο classifier
    «μαθαίνει» το BENIGN ως μία class αλλά πρακτικά πρέπει να μάθει
    πολλά distinct flavors.
    \item \textbf{Outliers / unknown attacks.} Το DBSCAN σημείωσε
    \(12.573\) rows ως noise - μερικά μπορεί να είναι zero-day
    attacks ή corrupted flows που δεν ταιριάζουν σε γνωστή κατηγορία.
    \item \textbf{Γεωμετρική επιβεβαίωση των features.} Το silhouette-best
    \(k\) ισούται με τον αριθμό των true labels (15). Αυτό είναι «δωρεάν»
    επιβεβαίωση ότι τα 47 selected features περιέχουν αρκετή information
    για να διακρίνουν τις attack categories - χωρίς ποτέ να είδαμε labels.
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{lr}
        ...
        \textbf{Μέγεθος}                                  & \textbf{Τιμή} \\
        \midrule
        Subsample (cap = 25K)                             & 134.187 rows  \\
        PCA components (\(\geq 85\%\) variance)           & 17            \\
        Hierarchical subsample                            & 2.468 rows    \\
        K-Means k-search                                  & \(k \in [2,15]\) \\
        K-Means best k (silhouette)                       & 15            \\
        K-Means metrics                                   & sil 0,4237 / DB 0,8278 \\
        Hierarchical best linkage                         & average       \\
        Hierarchical metrics                              & sil 0,7334 / DB 0,2185 \\
        DBSCAN sweep                                      & 5 \(\times\) 3 = 15 combos \\
        DBSCAN best (silhouette)                          & \(\varepsilon=0{,}3, ms=20\) \\
        DBSCAN best metrics                               & sil 0,4480 / DB 0,6354 \\
        DBSCAN noise points (best)                        & 12.573 (\(\sim 9{,}4\%\)) \\
        \bottomrule
    \end{tabular}
    \caption{Βασικά αποτελέσματα του Q3.}\label{tab:q3_summary}
\end{table}
...
\begin{enumerate}
    \item \textbf{Decision Tree και Random Forest είναι σχεδόν ισόπαλα}
    στο top-level. RF B κερδίζει οριακά στο F1-weighted (\(0{,}9921\))·
    RF A κερδίζει στο F1-macro (\(0{,}9116\)). Το LR υστερεί σημαντικά
    στο macro (\(0{,}781\)) επειδή οι attack classes \emph{δεν} είναι
    γραμμικά διαχωρίσιμες στο 47-feature space - οι rare attacks
    χρειάζονται non-linear decision boundaries.
    \item \textbf{Το split-vs-CV protocol δίνει ίδια best params για το
    LR} (μεγάλο val set αρκεί), αλλά \emph{διαφορετικά} για DT και RF
    (CV προτιμά unbounded \texttt{max\_depth}). Σε γενικές γραμμές, η
    CV είναι πιο αξιόπιστη όταν τα data είναι περιορισμένα ή υπάρχουν
    rare classes.
    \item \textbf{Η \texttt{Destination Port} είναι το μόνο feature
    που dominate ένα μεμονωμένο DT} (importance \(\sim 0{,}39\)). Στο
    RF η importance εξομαλύνεται μέσω bagging - top-5 features έχουν
    importance \(0{,}04\)-\(0{,}07\). Αυτή η εξισορρόπηση είναι το
    χαρακτηριστικό signature του ensemble averaging.
    \item Configurable mode (\texttt{SUBSAMPLE\_CAP}): \texttt{25\_000}
    (fast, \(\sim 134\)K rows, \(\sim 15\) min) / \texttt{100\_000}
    (medium, \(\sim 500\)K rows, \(\sim 2\)-\(4\) hours) / \texttt{None}
    (full \(\sim 2{,}5\)M rows, \(\sim 12+\) hours). Το LR με solver
    \texttt{saga} είναι ο κύριος bottleneck σε full mode.
\end{enumerate}

\subsection{Q3 - Unsupervised Clustering}
\begin{enumerate}
    \item \textbf{17 PCA components} καλύπτουν \(\sim 87\%\) της variance.
    Δεν υπάρχει ένα κυρίαρχο axis - η information είναι κατανεμημένη.
    \item \textbf{Το silhouette-best \(k\) για το K-Means είναι ακριβώς 15}
    - ίσο με τον αριθμό των γνωστών labels. Αυτή η σύμπτωση είναι ισχυρό
    σήμα ότι η ενδογενής γεωμετρία των δεδομένων αντιστοιχεί στις attack
    categories (γεωμετρική επιβεβαίωση του feature engineering του Q1).
    \item \textbf{Hierarchical με average linkage} έδωσε τα καλύτερα
    metric numbers (silhouette \(0{,}733\), DB \(0{,}219\)), αν και
    εκπαιδεύτηκε σε \(2.468\)-row stratified subsample λόγω του
    \(O(n^2)\) memory cost (η πλήρης distance matrix για \(134\)K rows
    είναι \(\sim 72\) GB).
    \item \textbf{DBSCAN ταξινομεί \(\sim 9{,}4\%\) του dataset ως
    noise} (best config: \(\varepsilon=0{,}3, \texttt{min\_samples}=20\)).
    Αυτά τα noise points είναι candidate \emph{outliers / unknown
    attacks} - πληροφορία που το supervised setting του Q2 δεν παράγει.
...
\begin{itemize}
    ...
    \texttt{SUBSAMPLE\_CAP} ώστε ο χρήστης να επιλέγει trade-off
    speed-vs-fidelity χωρίς αλλαγές σε άλλο μέρος του κώδικα.
    \item \textbf{Διαχωρισμός responsibilities.} Reusable helpers ζουν
    στο \texttt{utils.helpers} (15 functions, re-exported μέσω
...
\begin{itemize}
    ...
    να βελτιώσουν τη DBSCAN υλοποίηση.
...
notebook και προσαρμόστε τη μεταβλητή \texttt{SUBSAMPLE\_CAP} στο
preferred mode (\texttt{25\_000} = fast, \texttt{100\_000} = medium,
\texttt{None} = full).
